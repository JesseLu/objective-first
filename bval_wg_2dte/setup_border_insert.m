function [Ahat, bhat, add_border, rm_border] = setup_border_insert(A, x)
...
bhat = A(:,bor) * x(bor);
    ...
    %
    % Simple functions to remove and re-insert the border values.
    %
...
rm_border = @(x) sparse(1:n_int, int, ones(n_int, 1), n_int, 3*N) * x;
